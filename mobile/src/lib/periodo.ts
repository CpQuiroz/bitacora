// Espejo de web/src/lib/periodo.ts (que a su vez espeja
// backend/src/routes/dashboard.ts → resolverPeriodo) — mismo formato de
// fecha y mismos valores de período en las dos plataformas, para que
// Informes muestre siempre el mismo rango que calcula el backend.
...
export function resolverPeriodo(periodo: PeriodoValor, desdePersonalizado?: string, hastaPersonalizado?: string): { desde: string; hasta: string } {
  const hoy = new Date();
