...
**Última migración aplicada a producción: `66` (2-sep-2026, `supabase db push` con el CLI linkeado a prod; tracking `remote:66` OK, CI de migraciones en verde).** Actualizar este número a mano cada vez que se aplique una migración nueva a prod.
> `64` (check-in/out geolocalizado en `ordenes_servicio`), `65` (`usuarios.funcion`), `66` (`viajes.origen_captura` suma `'app'`) — para la reconstrucción de la app móvil (Fase 2).

> ⚠️ **El tracking de migraciones de DEV (`pruwvpnlvrvgtmpetlsr`) está desincronizado a partir de la 62.** La `62_tareas_trabajo_id.sql` se aplicó a dev por el SQL Editor; la `63_whatsapp_conversaciones.sql` se aplicó a dev con `supabase db query --linked --project-ref pruwvpnlvrvgtmpetlsr -f ...` (Management API, usa el access token de `supabase login`, no necesita el password de la DB). En ambos casos el schema de dev **sí** quedó actualizado, pero `supabase_migrations.schema_migrations` en dev **no** tiene esas filas. Consecuencia: si algún día se linkea el CLI a dev y se corre `db push`, va a intentar re-aplicarlas y fallar con "ya existe". Fix cuando pase: `supabase migration repair --status applied 62 63 64 65 66` (con el CLI linkeado a dev) antes del `db push`. (64/65/66 se aplicaron a dev el 2-sep-2026, también por Management API.)
