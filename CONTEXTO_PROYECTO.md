...
>
> **Actualizado 6-sep-2026.** La app **ya está publicada en internet** (Vercel + Render +
...
> **94 migraciones aplicadas en prod y dev** (salvo la fila de tracking de la 94, ver
> abajo). App móvil en **1.7.0 / versionCode 14**.
>
> ---
>
> **Novedades del 4-6 sep (reestructuración de navegación + fixes de terreno — todo en
> producción; el móvil por APK local, ver más abajo):**
>
...
| Mobile | Expo + React Native | Expo ~57.0.19, RN 0.86.3, React 19.2.3. Nav: `@react-navigation` v7. Auth extra: `expo-local-authentication` (bloqueo con huella/Face ID), `expo-auth-session` + `expo-web-browser` + `expo-crypto` (login con Google, PKCE). `expo-file-system` ~57 (persiste las fotos de la cola de sync). `expo-image-manipulator` (compresión). Sin `expo-maps` (se quitó — ver App móvil en sección 4). Build normal vía **EAS** (`mobile/eas.json`, perfiles development/preview/production; `preview` = APK interno apuntando a producción); mientras la cuota EAS Free está agotada (reset 1-oct), **build local** (`expo prebuild` + `./gradlew assembleRelease`, JDK 17 + Android SDK/NDK vía Homebrew) — firmado con keystore de debug |
...
│   └── src/
│       ├── routes/             52 archivos, un router por recurso (incluye accesos.ts —
...
│   │                           Hoy · Agenda · Clientes · Más. Build por APK local
│   │                           (`./gradlew assembleRelease`) mientras la cuota EAS Free
│   │                           está agotada.
│   └── src/                     shell/navigation/ (HoyStack, MasStack, AppTabs sin
│                                tabsPara), features/ (hoy/, mas/, agenda/CatalogoScreen…),
│                                services/ (cola de sync offline; fotoCola.ts persiste las
│                                fotos), lib/
...
└── supabase/
    ├── migrations/              94 archivos SQL numerados, se aplican en orden
...
Esquema real (consultado en vivo, prod, 6-sep-2026: **79 tablas, 94 migraciones aplicadas** — la fila de tracking de la 94 pendiente, ver header). Las tablas de tenant tienen `empresa_id` y policy RLS `empresa_id = empresa_actual()` (recordatorio: esa policy no se ejecuta contra el tráfico real — ver la fila "Multi-tenant" de la sección 1). No son tablas de tenant:
- `empresas` misma,
...
- **Estado del despliegue (6-sep-2026):** la app **ya está en producción** (Vercel + Render + Supabase prod + Resend + Cloudflare, ver `docs/PUESTA_EN_PRODUCCION.md`). Migraciones 1-94 aplicadas en prod y dev (la fila de tracking de la 94 en prod está pendiente — `migration repair --status applied 94`). Workflow `keep-warm.yml` mitiga el arranque en frío de Render. Empresa "Transportes Itineris" **cambió de rubro `transporte` a `cosmetologia`** en prod (usa Agenda Pro / reservas, tema visual "Vino y eucalipto", pantalla "Nueva reserva" exclusiva); módulos `agenda`, `viajes`, `agenda_pro`, `asistente`, `informe_ia` activados; 3 usuarios (1 admin, 2 colaboradores). **Auto-deploy de Render**: no siempre se dispara solo por push — se usa el Deploy Hook (`api.render.com/deploy/srv-daatkjf10e5c73cnrb30?key=…`). **Clasificador de Claude Code**: permite consultas READ-ONLY a la DB de prod (`supabase db query --linked`) pero BLOQUEA writes/DDL — las migraciones a prod las corre la usuaria. Lo que falta para dejarla operativa:
...
- **Del prompt original, no iniciado o incompleto:**
  ...
  - App móvil: arquitectura + auditoría + PASO 3 (4 pestañas) + fixes de terreno + APK 1.7.0 instalado en dispositivo. **Falta**: piloto con estilistas/choferes reales; build de producción/`.aab` para Play Store; que el usuario configure el login con Google; **Sentry en el móvil** (plugin incompatible con SDK 57); **resolver la cuota de EAS** (agotada hasta 1-oct — hoy se compila local con keystore de debug, que no actualiza encima de un APK de EAS).
- **Fuera del prompt original, flagged durante el desarrollo como fuera de alcance/pendiente:**
  ...
  - **Tracking de la migración 94 en prod** — la columna `tareas.adicionales` está aplicada pero falta `supabase migration repair --status applied 94 --project-ref yjbskbskyadxjooxngjv` para que el CI `check-migraciones-prod.yml` deje de fallar.
