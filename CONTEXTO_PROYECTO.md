...
>
> **Actualizado 20-sep-2026.** La app **ya está publicada en internet** (Vercel + Render +
...
> **112 migraciones, todas aplicadas y trackeadas en prod.** App móvil en **1.10.7 /
> versionCode 48** — pero ojo: el código de mobile sigue una tanda por delante del último
> build real (folios de Cliente/Cobro en las pantallas de mobile, ver más abajo, se
> pushearon después de compilar la 1.10.7) — hay un build nuevo pendiente al momento de
> escribir esto.
>
> **Novedades del 13 al 20-sep (todo en producción salvo lo del móvil, que espera el
> próximo build — resumen; el detalle línea por línea de cada commit vive en
...
Esquema real (consultado en vivo, prod, 12-sep-2026: **87 tablas** —84 + `levantamientos`/
`levantamiento_materiales`/`levantamiento_fotos`—; **112 migraciones, todas aplicadas y
trackeadas en prod** al 20-sep — la 101-112 son todas aditivas, sin tablas nuevas: folios
propios (Cita/Viaje/Levantamiento en la 108; Cliente/Pack de sesiones/Gasto/Proveedor/
Cobro en la 112), `empresas.tema` + su constraint (109/110), `levantamientos.fecha_visita`
(111), entre otras).
...
- **Del prompt original, no iniciado o incompleto:**
  ...
  - App móvil: arquitectura + auditoría + PASO 3 (4 pestañas) + fixes de terreno + galería/eliminar fotos + fixes de sync + safe-area + tema por empresa + folios + APK 1.10.7/vc48 (20-sep) probado en dispositivo real contra producción. **Falta**: piloto con estilistas/choferes reales; build de producción/`.aab` para Play Store; que el usuario configure el login con Google; **Sentry en el móvil** (plugin incompatible con SDK 57); **resolver la cuota de EAS** (agotada hasta 1-oct — hoy se compila local con keystore de debug, que no actualiza encima de un APK de EAS); **build nuevo pendiente** al momento de escribir esto (el folio de Cliente/Cobro en mobile se pusheó después del último build).
