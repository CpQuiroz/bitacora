...
>
> **Actualizado 1-sep-2026.** La app **ya está publicada en internet** (Vercel + Render +
...
| CI/CD | **No implementado.** Sin GitHub Actions, sin Terraform activo. Los deploys son automáticos por push a `main` (Vercel y Render escuchan el repo) pero no hay lint/typecheck/test previo | — |
...
│       │                       gate de 2FA obligatorio por rol, gate de trial vencido (ver
│       │                       sección 4), gatea por módulo (rol × empresa_modulos)
...
└── supabase/
    ├── migrations/              59 archivos SQL numerados, se aplican en orden
...
Esquema real (consultado en vivo: **63 tablas, 113 foreign keys, 59 migraciones aplicadas**). Todas tienen `empresa_id` y policy RLS `empresa_id = empresa_actual()` (recordatorio: esa policy no se ejecuta contra el tráfico real — ver la fila "Multi-tenant" de la sección 1), salvo:
- `empresas` misma,
...
- **Estado del despliegue (1-sep-2026):** la app **ya está en producción** (Vercel + Render + Supabase prod + Resend + Cloudflare, ver `docs/PUESTA_EN_PRODUCCION.md`). Lo que falta para dejarla operativa:
  - Dar de alta la primera empresa cliente ("Transportes Itineris") de punta a punta (crear empresa → invitación por correo → activar cuenta del admin → entrar al dashboard).
...
- **Del prompt original, no iniciado o incompleto:**
  - CI/CD (GitHub Actions) — cero configuración. Los deploys son automáticos por push a `main` pero sin lint/typecheck/test previo.
