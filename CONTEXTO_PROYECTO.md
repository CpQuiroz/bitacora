...
└── supabase/
    ├── migrations/              41 archivos SQL numerados, se aplican en orden
...
| Notificaciones al Cliente + Portal de Cliente | ✅ Implementado | Correos automáticos al cliente (cotización enviada/por vencer, técnico en camino, OS completada, cobro pendiente/vencido, cita agendada) con mensaje y asunto personalizables por tipo, switches on/off en Configuración → Notificaciones, historial de envíos con reintento manual. El correo incluye un link temporal (`portal_accesos`) a un Portal de Cliente propio (`/portal/*`, sin cuenta de Bitácora, login recurrente por código de 6 dígitos) donde el cliente ve/descarga OS y cotizaciones en PDF, aprueba/rechaza cotizaciones, confirma/cancela citas de Agenda Pro y revisa sus cobros |
...
Esquema real (consultado en vivo, 52 tablas, 97 foreign keys). Todas tienen `empresa_id` y policy RLS `empresa_id = empresa_actual()` (recordatorio: esa policy no se ejecuta contra el tráfico real — ver la fila "Multi-tenant" de la sección 1), salvo:
- `empresas` misma,
...
| Agenda | `tareas`, `paquetes_sesiones` | `tareas.cliente_id → clientes`, `.responsable_id → usuarios`, `.paquete_id → paquetes_sesiones` (nullable); `paquetes_sesiones.cliente_id → clientes` — saldo de sesiones siempre calculado, nunca columna propia |
...
- **Fuera del prompt original, flagged durante el desarrollo como fuera de alcance/pendiente:**
  ...
  - Personalización del mensaje de "cita agendada" (Agenda Pro) por WhatsApp — hoy el mecanismo de aviso es solo por correo (`notificarCliente`), igual que cotizaciones y OS completada; no hay envío de citas por WhatsApp.
