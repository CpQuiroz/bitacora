...
>
> **Actualizado 21-sep-2026.** La app **ya está publicada en internet** (Vercel + Render +
...
> **115 migraciones, todas aplicadas y trackeadas en prod** (verificado en vivo).
> App móvil en **1.10.8 / versionCode 49** — el código de mobile sigue una tanda por
> delante del último build real (Levantamiento con fecha+hora de visita en Pizarra/
> Agenda, ver más abajo, se pusheó después de compilar la 1.10.8) — hay un build nuevo
> pendiente al momento de escribir esto.
>
> **Novedades del 20 al 21-sep (todo en producción — resumen; el detalle línea por línea
> de cada commit vive en `progress/current.md` y `progress/history.md`, no acá):**
> - **Unificación "Tipo de OS" + "Tipo de Trabajo" → "Tipo de OS/Trabajo"** (migración 115,
>   aplicada): el formulario de Nueva OS mostraba 2 selectores casi idénticos uno debajo
>   del otro — la migración 15 los había separado a propósito en su momento (campos
>   dinámicos del formulario vs. color/checklist/tiempo estimado), pero a la luz del uso
>   real (0 nombres repetidos entre ambas tablas por empresa, solo 1 `trabajo` en toda la
>   base con ambos ids seteados) convenía fusionarlos. La antigua `tipos_trabajo` ganó las
>   columnas de `tipos_os` y se renombró a **`tipos_os_trabajo`**; `trabajos.tipo_trabajo_id`/
>   `tipo_os_id` → **`tipo_id`** único; `tipos_os` se dropeó. Tocó ~15 archivos (shared:
>   `TipoOsTrabajo` único; backend: nuevo `routes/tiposOsTrabajo.ts` en
>   `/api/tipos-os-trabajo`; web: una sola página de Configuración con un solo form,
>   `ordenes/nueva` con un solo picker; mobile: `services/trabajos.ts`/
>   `TrabajoDetalleScreen.tsx`).
> - **Levantamiento gana `hora_visita`** (migración 114, texto `"HH:MM"` — mismo criterio
>   que `tareas.hora`, no `time`, para no arrastrar el sufijo de segundos que devuelve
>   Postgres al leer) junto a `fecha_visita` (migración 111, 20-sep) — visible en
>   alta/edición/detalle/tabla (web) y en Pizarra + Agenda (mobile, antes hardcodeaban
>   `hora: null`/`"--:--"`).
> - **Agenda (web) gana un menú "+ Nuevo"**: antes solo "Nueva Tarea" (cita); ahora
>   Cita/Orden de servicio/Levantamiento, reutilizando el "volver a Agenda" que ya existía
>   medio armado para OS y un `?crear=1` nuevo para abrir el modal de Levantamientos solo.
>   El form rápido de "Nueva Tarea" en sí también ganó un botón **"Crear Levantamiento"**
>   junto al ya existente "Crear Orden de Servicio" — a diferencia de la OS (que vincula
>   `trabajo_id` a la tarea al volver), `tareas` no tiene columna equivalente para
>   Levantamiento, así que lo ya tipeado (cliente/descripción/fecha/hora) se lleva
>   precargado vía query params al form de Levantamientos, sin ida y vuelta.
> - **Tipos de documento gana botón Eliminar** (el backend ya tenía el DELETE con 409
>   "desactivar" si está en uso, solo faltaba la UI) y se corrigió un bug real: las
>   sugerencias por rubro desaparecían apenas se creaba el primer tipo (gating por "lista
>   vacía" en vez de "quedan sugerencias sin usar") — mismo bug encontrado y corregido en
>   Tipos de OS/Trabajo y Categorías de Gasto, los 3 catálogos que comparten el mecanismo.
> - **Personas/Perfiles/Plantillas** (Configuración web): invitar a alguien pasa de
>   formulario siempre-abierto a botón + modal; roles de Perfiles y permisos pasan a
>   acordeón (plegados, se expanden solos si tienen cambios sin guardar); "Texto de
>   encabezado" de Plantillas pasa de un input de una línea a bloques con nivel
>   (Título/Subtítulo/Texto chico/Párrafo) — migración 113, `jsonb`, backfill a un bloque
>   "parrafo" sin perder contenido (no había ninguna fila con contenido real en prod).
> - **Folio correlativo nuevo** para Cliente/Pack de sesiones/Gasto/Proveedor/Cobro
>   (migración 112, prefijos CLI/PACK/GTO/PROV/COB) — Persona quedó afuera a pedido
>   explícito (ya tiene RUT).
> - **Portal del cliente** se movió del sidebar top-level (grupo Clientes) a ser una
>   pestaña más de Configuración.
> - **Fix real de Inventario**: el switch de "activar Inventario" (Configuración) se
>   deshabilitaba con un chequeo de "¿hay productos en Catálogo?" que corría UNA SOLA VEZ
>   al montar la página, sin re-fetch — si el catálogo estaba vacío cuando esa pestaña
>   cargó por primera vez, el switch quedaba pegado en "sin productos" para siempre en
>   esa sesión aunque después se agregaran productos reales. Se sacó el bloqueo duro; el
>   aviso de "crea un producto primero" queda como sugerencia, no como condición.
>
> **Novedades del 13 al 20-sep (todo en producción — resumen; el detalle línea por línea
> de cada commit vive en `progress/current.md` y `progress/history.md`, no acá):**
> - **Identidad visual renovada**: ícono nuevo de la app (libro abierto estilizado, azul
>   `#2563a6`) reemplaza al genérico de Expo — mismo ícono en mobile (adaptive icon
>   Android) y en el favicon/`icon.png` de referencia. Investigación previa de tendencias
>   de color de mercado (con fuentes citadas) antes de proponer paleta — la usuaria eligió
>   **"azul confianza"**.
> - **Segundo y tercer tema visual seleccionable por empresa** (`empresas.tema`, migraciones
>   109/110): además de **"Faena"** (la paleta crema que reemplazó al viejo azul/IBM Plex —
>   sección 3, incluida por completo desde antes de esta tanda), ahora existen **"Taller"**
>   (naranja `#d1580f` + tipografía Archivo/IBM Plex Sans distinta — único tema que cambia
>   tipografía) y **"Confianza"** (azul `#2563a6`, misma tipografía que Faena — Caprasimo/
>   Figtree). Selector en Configuración → Empresa (web). **Portado a mobile también**
>   (nuevo, 20-sep): mobile no tiene cascada CSS como la web, así que en vez de portar el
>   mecanismo 1:1 (hubiera significado tocar los ~800 usos de `tokens.color.*` repartidos
>   en 72 archivos), se muta el objeto compartido `tokens.color` in-place al detectar el
>   tema de la empresa logueada (`mobile/src/theme/aplicarTema.ts`), aplicado
>   sincrónicamente en el render (no en un efecto, para no pintar un frame con colores
>   viejos) y con un remount completo de la navegación al cambiar de tema en caliente.
>   Selector nuevo en Perfil (mobile no tenía ninguna pantalla de config de empresa antes),
>   solo-color (Taller no cambia de tipografía en mobile — precarga fija de Caprasimo/
>   Figtree, agregar más fuentes es un cambio aparte no pedido).
> - **Fix crítico de deploy (Vercel) encontrado y corregido**: el build de producción
>   llevaba **~1 día fallando en silencio** (push a `main` no implica deploy exitoso —
>   lección aprendida, ahora se verifica el estado real en el dashboard de Vercel después
>   de cada push relevante). Causa: `recharts@3.10.1` declara `@reduxjs/toolkit`/
>   `react-redux` como dependencias reales propias, y la resolución de npm workspaces no
>   las encontraba transitivamente. Fix: agregarlas como dependencias **directas** de
>   `web/package.json`.
> - **Levantamientos ganó fecha de visita** (`levantamientos.fecha_visita`, migración 111,
>   date nullable) — antes no tenía ningún campo de fecha, así que un levantamiento
>   asignado a un técnico aparecía siempre "pendiente sin día fijo". Con fecha asignada:
>   aparece en el día correcto en la **Pizarra** (mobile) y — nuevo — en la pestaña
>   **Agenda** de mobile (que hasta ahora solo mostraba citas; primera vez que integra otro
>   tipo de ítem, con color distinto y navegando al detalle propio del levantamiento al
>   tocarlo). Sin fecha, sigue el comportamiento de siempre. Header del detalle (web+mobile)
>   separó el folio (antes iba pegado al texto largo del estado y quedaba "perdido" al
>   envolver a varias líneas) del estado (ahora un `StatusBadge` propio).
> - **Folio correlativo nuevo para Cliente, Pack de sesiones, Gasto, Proveedor y Cobro**
>   (migración 112, mismo mecanismo de contador atómico por empresa que ya usaban OS/Cita/
>   Viaje/Levantamiento) — prefijos `CLI-`/`PACK-`/`GTO-`/`PROV-`/`COB-` (este último
>   deliberadamente NO "FAC-": `facturas` es un registro de cobro interno, no una factura
>   tributaria real con folio SII/CAF). **Persona (`usuarios`) se dejó sin folio a
>   propósito** — ya tiene RUT como identificador único, sería redundante. Antes de tocar
>   código se investigó el modelo de "Pack de sesiones" (la usuaria pedía arquitectura
>   plantilla+clon) y se confirmó que **ya existe exactamente eso** (`TipoPack` catálogo →
>   `PaqueteSesiones` instancia única con snapshot al vender, y `NuevaCitaScreen` ya clona
>   un `TipoPack` al vuelo si hace falta) — nada que rediseñar ahí, solo le faltaba folio.
> - **Sidebar web**: Levantamientos pasó a listarse antes que Órdenes de servicio (el
>   flujo real es evaluar en terreno primero, la OS nace después al aprobar la cotización)
>   — excepción puntual al criterio general de "orden por frecuencia de uso" del PASO 4.
> - **Mobile — safe-area + homologación de tarjetas**: `AppTabs.tsx` (tab bar) ganó
>   `paddingBottom: insets.bottom` — Android edge-to-edge (Expo SDK 57) no reservaba solo
>   ese espacio, y varias pantallas (Pizarra, Más) quedaban tapadas por la barra de gestos.
>   5 pantallas modales (fuera del pager de tabs) necesitaron su propio fix aparte.
>   `QuickAccessCard` (`packages/ui/src/native`) extraído del código ya-consistente de "Más
>   → Accesos rápidos" para que sea reutilizable (auditado el resto de la app: solo
>   `BotonGrande` en Mantención se parece, pero es un patrón deliberadamente distinto —
>   no se forzó a compartir componente).
> - **Selector de día unificado** (`SelectorDias`, `packages/ui/src/native`): Nuevo gasto y
>   Nuevo trabajo arrancaban la tira de días 7 atrás — al abrir el formulario lo visible
>   eran días pasados, hoy quedaba fuera de pantalla. Ahora hace scroll a hoy al montar y
>   marca "HOY" siempre (esté o no seleccionado), sin perder la posibilidad de elegir una
>   fecha pasada. Aplicado también a Nueva cita (que ya arrancaba en hoy, solo le faltaba
>   la marca). La Agenda de mobile (tab marcada como congelada en las reglas del proyecto)
>   se tocó igual, por pedido explícito — la excepción que la regla misma prevé.
> - **Nuevo gasto (mobile)**: reordenado (descripción primero, foto al final); Proveedor y
>   Categoría ahora dejan crear uno nuevo sin salir del formulario (Categoría gateada por
>   el mismo módulo `configuracion` que ya la protege en el backend — un colaborador no ve
>   esa opción); el picker de Orden de Servicio ahora muestra el folio (antes solo el
>   nombre del cliente, indistinguible entre varias OS del mismo cliente).
> - **Fix de flash de pantalla vacía** en `/dashboard/levantamientos` (web) — el
>   `<DashboardShell>` se montaba después de los datos en vez de antes, mismo patrón ya
>   corregido en Personas/Órdenes.
>
> ---
>
...
>   shell/sidebar misma) están migradas y verificadas, ~40 commits. **Paso 7 cerrado el
>   12-sep** (regla ESLint anti-hex/px, Storybook, doc enlazado desde `CLAUDE.md` — el
>   check de CI ya estaba cubierto por la tarea de CI general). Detalle completo en
>   `docs/design-system.md` y `docs/design-audit.md`.
...
> `empresa_modulos`, apagado por defecto; **aplicada en dev y en prod**, verificado en
> vivo — ver más abajo el detalle del gotcha de `--include-all`):
...
>
> **Novedades del 12-sep — Backlog técnico cerrado (5 de 6 ítems; detalle en la sección
> "Estado del despliegue" y secciones 3/7):** CI real (`verificar.yml`, `./verificar.sh`
> completo en cada push/PR, sin secrets), lint de `web` arreglado (bug de hoisting de
> npm workspaces), backend con su primer test (smoke de arranque real), Deploy Hook de
> Render rotado, y el Paso 7 del sistema de diseño (regla de ESLint anti-hex/px +
> Storybook para `packages/ui`, 16 historias). Queda 1 solo ítem abierto: el E2E de
> Mantención de flota en prod, bloqueado en que alguien cierre una OS desde el APK en un
> teléfono real (no hay vista web equivalente para check-in/firma del técnico).
>
> ---
>
...
## 3. Sistema de diseño ("crema" — reemplazó a "Faena", + 2 temas nuevos por empresa)

Migración completa (Pasos 0-7 cerrados, ~45 commits, 9-12 sep) de la dirección visual
original ("Faena": azul tinta `#1e4e8c` / IBM Plex / esquina corta) a una nueva, aplicada
a **toda la web** (las 7 pantallas del sidebar + la shell) y a mobile.

**⚠️ Colisión de nombre a propósito de aclarar**: la paleta CREMA descrita abajo (fondo
`#f5ead8`, acento `#c67139`) es la que reemplazó por completo a la vieja "Faena"
(navy/IBM Plex) — pero cuando después (13-20 sep) se agregó el selector de tema **por
empresa** (`empresas.tema`), a la opción por defecto de ese selector se la llamó
**"Faena"** también, aunque ya es esta paleta crema, no la navy original. Las otras 2
opciones del selector son **"Taller"** (naranja `#d1580f`, tipografía Archivo/IBM Plex
Sans distinta — el único de los 3 que cambia tipografía además de color) y
**"Confianza"** (azul `#2563a6`, misma tipografía que "Faena" — Caprasimo/Figtree). Los
3 viven como bloques `[data-tema="x"]` generados desde `tokens.json`
(`colorTaller`/`fontTaller`, `colorConfianza`) — ver más abajo. En mobile, el mecanismo es
distinto (no hay cascada CSS): se muta `tokens.color` in-place
(`mobile/src/theme/aplicarTema.ts`) al detectar `empresa.tema`, aplicado sincrónicamente
en el render de `NavegacionConTema` (`shell/App.tsx`) — y solo color, Taller no cambia de
tipografía en mobile (agregar Archivo/IBM Plex Sans ahí es un cambio más grande, no
pedido). Selector: Configuración → Empresa (web), Perfil (mobile, admin-only — mobile no
tenía ninguna pantalla de config de empresa antes de esto).

**Paso 7 (12-sep,
anti-degradación)**: regla de ESLint `bitacora/no-literal-color-or-px` (`web/eslint-
rules/anti-token.mjs`, solo web — mobile no tiene ESLint configurado) marca color hex/rgb
y espaciado arbitrario de Tailwind en px antes de llegar a CI, con el mismo criterio y
lista de exentos que `scripts/check-colores.mjs`; el check de CI ya estaba cubierto por
el workflow `verificar.yml` (ver sección 7); y **Storybook para `packages/ui`**
(`npm run storybook -w packages/ui`, 16 historias — una por primitiva de `src/web`,
cubriendo estados reales, no solo el caso feliz). **2 seams quedan fuera a propósito**,
sin fecha (decisión ya tomada, no bloqueada por nada técnico): contenido de **Agenda**
(`dashboard/agenda/*`, usa `DataTable`/`EstadoCitaRiel` propios) y de **`/superadmin`**
(shell migrado, contenido sigue en Faena) — ninguno era parte de los 7 grupos de nav del
pedido original "homologar todo".
...
- **Consumo:**
  | Plataforma | Cómo |
  |---|---|
  | Web (Next + Tailwind v4) | `@import "@bitacora/design-tokens/tokens.css"`. Clases con prefijo **`ds-`** (`bg-ds-bg`, `text-ds-h1`, `rounded-ds-pill`, `shadow-ds-md`…) — el prefijo existe para convivir con los últimos restos de Faena (Agenda, `/superadmin` — ver arriba) y se retira cuando esos 2 seams se migren, no antes. |
...
| **Levantamientos** (nuevo, migración 100) | ✅ Implementado — **opt-in por empresa** (`empresa_modulos`, apagado por defecto — ninguna empresa lo tiene activo hoy) | Admin crea una Orden de Levantamiento (cliente + técnico + qué evaluar) → el técnico asignado completa en terreno desde mobile (descripción + materiales del Catálogo con cantidad, sin tocar stock, + fotos; visible solo si `usuarios.funcion ∈ {tecnico, chofer}`, primer uso real de ese campo) → Admin cotiza **fuera de Bitácora** (`referencia_externa` texto libre) → al aprobar, nace automáticamente una OS que hereda los materiales como `os_items` (el descuento de stock ocurre ahí, por el mecanismo existente de `estado_os`, sin gancho nuevo). Cola offline en mobile (mismo patrón que Mantención/Viajes/OS). Web: `/dashboard/levantamientos` (Admin: crear/editar/cotizar/aprobar/rechazar/eliminar, fotos). Deliberadamente separado de Cotizaciones (sigue igual para el resto de empresas) |
...
Esquema real (consultado en vivo, prod, 21-sep-2026: **87 tablas**, **115 migraciones,
todas aplicadas y trackeadas**. La 115 (unificación Tipo de OS/Trabajo) dropeó `tipos_os`
y renombró `tipos_trabajo` a `tipos_os_trabajo` — de ahí que el conteo neto de tablas
bajó de 88 a 87 pese a que siguen sumando migraciones. Del 101 al 115: todas aditivas
salvo la 113 (cambia el tipo de una columna, sin tabla nueva) y la 115 (dropea una tabla
y renombra otra) — folios propios (Cita/Viaje/Levantamiento en la 108; Cliente/Pack de
sesiones/Gasto/Proveedor/Cobro en la 112), `empresas.tema` + su constraint (109/110),
`levantamientos.fecha_visita`/`hora_visita` (111/114), `plantillas_documento.
texto_encabezado` texto→jsonb (113), Tipo de OS/Trabajo unificado (115), entre otras).
**Las 87 tienen RLS activo** (100%) — 63 con policy de tenant real (`empresa_id =
...
| **Levantamientos** (migración 100) | `levantamientos`, `levantamiento_materiales`, `levantamiento_fotos` | `levantamientos.cliente_id → clientes`, `.tecnico_id → usuarios` (nullable hasta asignar), `.creado_por → usuarios` (2 FKs distintas a `usuarios` — PostgREST necesita el hint de columna `usuarios!tecnico_id(...)` para desambiguar), `.estado` (`creado`\|`asignado`\|`completado`\|`cotizado`\|`aprobado`\|`rechazado`), `.referencia_externa` (texto libre, cotización fuera de Bitácora), `.folio` (migración 108), **`.fecha_visita`** (date, nullable, migración 111, 20-sep — cuándo debe ir el técnico; sin ella, aparece en Pizarra "pendiente sin día fijo" como siempre; con ella, aparece ese día en Pizarra y en la Agenda de mobile). `levantamiento_materiales.levantamiento_id → levantamientos`, `.catalogo_item_id → catalogo_items`, `.cantidad` — al aprobar se copian 1:1 a `os_items` de la OS nueva. `levantamiento_fotos.levantamiento_id → levantamientos` |
...
| Financiero | `presupuestos` (cotizaciones), `presupuesto_items`, `facturas` (Cobros, incluye `valor_recibido`/`observaciones_pago` de "Registrar Pago"), `gastos` (incluye `trabajo_id` opcional y `fecha_pago`), `gastos_fijos`, `categorias_gasto`, `centros_costo`, `proveedores` | `presupuestos.trabajo_id` (conversión a OS), `.estado` incluye `expirado` (ahora calculado, no solo visual); `facturas.viaje_ids`/`trabajo_ids` (arrays, agrupan varios en una factura). **Folio (migración 112, 20-sep)**: `facturas.folio` (prefijo `COB-` al mostrarlo — NO `FAC-`, es un registro interno, no una factura tributaria con folio SII/CAF; asignado tanto en el alta manual como en `generar_factura()` vía RPC), `gastos.folio` (`GTO-`), `proveedores.folio` (`PROV-`) — los 3 nullable, sin backfill histórico; la importación masiva (CSV) de clientes/proveedores NO asigna folio a propósito |
...
- **CI real (12-sep)** — hasta esta tanda solo existían `keep-warm.yml` (evita que Render duerma) y `check-migraciones-prod.yml` (compara migraciones locales vs. prod). `.github/workflows/verificar.yml` (nuevo) corre `./verificar.sh` completo en cada push a `main` y en cada PR — sin secrets, todo local al checkout. Primera corrida real encontró 1 bug genuino que la simulación local ("npm ci limpio") no había cazado: `tsc web` depende de `.next/types/**/*.ts` (rutas tipadas de Next 16, ej. `LayoutProps<"/">`), que Next genera solo con `next dev`/`build` y nunca está versionado — cualquier checkout 100% limpio sin ese paso previo fallaba. Fix: `verificar.sh` corre `npx next typegen` antes de `tsc web`.
- **Backend gana su primer test (12-sep)** — `backend/src/server.smoke.test.ts` (`node:test`, mismo patrón que `packages/shared`) levanta el `server.ts` REAL (mismo `app.listen`, puerto libre asignado por el SO) y prueba `/health` (200), `/api/me` sin token (401 real, sin tocar Supabase), y la verificación del webhook de WhatsApp (200 + eco del challenge / 403 con token equivocado — "ruta feliz" sin necesitar ninguna credencial real, ya que `requiereAuth` corta antes de llamar a Supabase y el webhook solo compara contra una env var).
- **`packages/ui` gana una regla de ESLint propia en vez de reusar `check-colores.mjs` tal cual** (`web/eslint-rules/anti-token.mjs`) — el script existente opera sobre texto crudo (across web/mobile/shared) y es el gate real de CI; la regla nueva opera sobre el AST (solo web, con ESLint ya integrado al editor/PR) y deliberadamente NO es un port 1:1 — comparten la misma lista de exentos (`scripts/colores-permitidos.json`) para no duplicar esa fuente de verdad. Se probó primero una versión amplia (marcaba cualquier `[Npx]` arbitrario de Tailwind) y encontró 109 falsos positivos contra patrones ya establecidos del sistema (`text-[11px]`, `rounded-[32px]`) — se acotó a solo utilidades de espaciado antes de integrarla.
...
- **Backlog activo (`trabajo_list.json` en la raíz del repo — fuente de verdad del día a día, más granular que esta lista). 5 de 6 ítems cerrados el 12-sep, queda 1 abierto:**
  1. ✅ **Eslint de `web`** — resuelto (bug de hoisting de npm workspaces entre `next` y `eslint-config-next`; `next` faltaba en `devDependencies` de la raíz).
  2. ✅ **CI que corre `./verificar.sh`** — `.github/workflows/verificar.yml`, push a `main` + PR, sin secrets.
  3. ✅ **Deploy Hook de Render rotado** — ver "Estado del despliegue" más abajo.
  4. ✅ **Backend con su primer test** — `server.smoke.test.ts` (ver sección 7).
  5. **E2E de Mantención de flota + PDF Fase 2 en prod — sigue parcialmente hecho, ÚNICO ítem del backlog todavía abierto:** verificado en vivo que un registro de mantención con foto se crea y genera PDF sin error, y que el Informe con IA funciona (RAG real, sin fotos/checklist responde honestamente que no hay datos en vez de inventar). **Bloqueado**: cerrar una OS con fotos por categoría + 2 firmas requiere check-in/fotos/firma del técnico, que **solo existe en mobile** — no hay ninguna vista web equivalente, así que ese último tramo lo tiene que hacer alguien con el APK en un teléfono real. La usuaria reportó el 12-sep que "sigue fallando al subir imágenes" en el APK 1.9.8 — **sin diagnóstico cerrado todavía**: el código ya comprime la foto antes de subir (`elegirFotos()`/`comprimirImagen()`, máx. 1600px / JPEG calidad 0.6, muy por debajo del límite de 10MB del backend) y ya tiene los 2 fixes de cola de sync de esta sesión (tareas #11/#12), así que un problema de *tamaño* de imagen es poco probable — pero falta el síntoma exacto (pantalla, mensaje, si el resto de la app queda respondiendo o se congela todo) para confirmar la causa real en vez de asumir.
  6. ✅ **Sistema de diseño — Paso 7** — regla ESLint anti-hex/px + Storybook + doc enlazado desde `CLAUDE.md` (el check de CI ya estaba cubierto por el ítem 2). Ver sección 3 y 7.
- **Estado del despliegue (12-sep-2026):** la app **ya está en producción** (Vercel + Render + Supabase prod + Resend + Cloudflare, ver `docs/PUESTA_EN_PRODUCCION.md`). **Prod tiene las 100 migraciones aplicadas y trackeadas** (la 99 —73 índices de cobertura de FK— y la 100 —módulo Levantamientos— corridas y validadas el 11 y 12-sep respectivamente). **Deploy Hook de Render rotado (12-sep)**: la key anterior había quedado en texto plano en una nota de memoria del agente (nunca en el repo/git) — se borró esa nota y se documentó la regla en adelante (`docs/PUESTA_EN_PRODUCCION.md` §3.4): esa key se pide a la usuaria cuando hace falta, nunca se guarda en repo/docs/memoria. **Gap de tracking en dev** (`pruwvpnlvrvgtmpetlsr`, detectado 11-sep): el *esquema* de dev está al día (todas las tablas/columnas hasta la 100 existen, verificado en vivo — se construyó y probó encima sin problema), pero `supabase_migrations.schema_migrations` de dev solo llega a la **74** — las migraciones 75+ se aplicaron con `supabase db query --linked -f <archivo>` en vez de `db push`, que no deja fila de tracking. No es urgente (dev funciona bien) pero un `migration repair` prolijo en algún momento evitaría confusión. Workflow `keep-warm.yml` mitiga el arranque en frío de Render. Empresa "Transportes Itineris" (`rubro='cosmetologia'` en la fila de `empresas` — el nombre de la empresa no define el rubro, verificado en vivo 11-sep; sigue siendo la misma empresa de prueba/piloto real de siempre, con Agenda Pro/reservas Y los módulos `agenda`/`viajes`/`ordenes_servicio`/Mantención de flota activos y usados de verdad) tiene datos de prueba mezclados con los reales — cliente interno **"Itineris Spa"** ya se usa como convención establecida para pruebas que no deben generar notificaciones a clientes de verdad (visto en varias OS y mantenciones de prueba existentes). **Auto-deploy**: Vercel se dispara solo por push a `main`; Render también (confirmado 11-sep, el Deploy Hook existe como respaldo pero no siempre hizo falta). **Clasificador de Claude Code**: permite consultas READ-ONLY a la DB de prod (`supabase db query --linked`) pero BLOQUEA writes/DDL — las migraciones a prod las corre la usuaria; **push a `main` (deploy de código) sí lo puede hacer el asistente**, con la usuaria al tanto dado el alcance (11-sep: 37 commits acumulados sin desplegar, incluido todo el rediseño visual, se empujaron juntos tras confirmarlo explícitamente). Lo que falta para estar 100% operativa:
  ...
  - **Rotar secretos que se expusieron en texto plano durante la puesta en marcha:** la primera Resend API key, un GitHub PAT, la `SUPABASE_SERVICE_ROLE_KEY` de producción (implica rotar el JWT secret del proyecto Supabase) — **la key del Deploy Hook de Render ya se rotó** (12-sep, ítem 3 del backlog).
...
- **Del prompt original, no iniciado o incompleto:**
  - CI/CD (GitHub Actions) — `check-migraciones-prod.yml`, `keep-warm.yml`, y desde el 12-sep `verificar.yml` (tsc × 6, tests, lint, `audit:tenant`, sistema de diseño, en cada push/PR — ver sección 7). Los deploys siguen siendo automáticos por push a `main`, ahora con ese gate corriendo en paralelo (no bloquea el deploy en sí, es un check informativo de PR/push).
  ...
  - App móvil: arquitectura + auditoría + PASO 3 (4 pestañas) + fixes de terreno + galería/eliminar fotos + fixes de sync + safe-area + tema por empresa + folios + APK 1.10.8/vc49 (20-sep) probado en dispositivo real contra producción. **Falta**: piloto con estilistas/choferes reales; build de producción/`.aab` para Play Store; que el usuario configure el login con Google; **Sentry en el móvil** (plugin incompatible con SDK 57); **resolver la cuota de EAS** (agotada hasta 1-oct — hoy se compila local con keystore de debug, que no actualiza encima de un APK de EAS); **build nuevo pendiente** al momento de escribir esto (la hora de visita del Levantamiento en Pizarra/Agenda se pusheó después del último build).
