...
> **115 migraciones — 114 aplicadas y trackeadas en prod, la 115 (unificación Tipo de
> OS/Trabajo) escrita y validada read-only pero pendiente de aplicar al momento de
> escribir esto.** App móvil en **1.10.8 / versionCode 49** — el código de mobile sigue
> una tanda por delante del último build real (Levantamiento con fecha+hora de visita en
> Pizarra/Agenda, ver más abajo, se pusheó después de compilar la 1.10.8) — hay un build
> nuevo pendiente al momento de escribir esto.
>
> **Novedades del 20 al 21-sep (todo en producción salvo lo del móvil y la migración 115,
> que están pendientes — resumen; el detalle línea por línea de cada commit vive en
> `progress/current.md` y `progress/history.md`, no acá):**
> - **Unificación "Tipo de OS" + "Tipo de Trabajo" → "Tipo de OS/Trabajo"** (migración 115,
>   pendiente de aplicar): el formulario de Nueva OS mostraba 2 selectores casi idénticos
>   uno debajo del otro — la migración 15 los había separado a propósito en su momento
>   (campos dinámicos del formulario vs. color/checklist/tiempo estimado), pero a la luz
>   del uso real (0 nombres repetidos entre ambas tablas por empresa, solo 1 `trabajo` en
>   toda la base con ambos ids seteados) convenía fusionarlos. `tipos_trabajo` gana las
>   columnas de `tipos_os`; `trabajos.tipo_trabajo_id`/`tipo_os_id` → `tipo_id` único;
>   `tipos_os` se dropea; `tipos_trabajo` se renombra a `tipos_os_trabajo`. Toca ~15
>   archivos (shared: `TipoOsTrabajo` único; backend: nuevo `routes/tiposOsTrabajo.ts`;
>   web: nueva página de Configuración con un solo form, `ordenes/nueva` con un solo
>   picker; mobile: `services/trabajos.ts`/`TrabajoDetalleScreen.tsx`).
...
Esquema real (consultado en vivo, prod, 21-sep-2026: **88 tablas** —114 migraciones
aplicadas y trackeadas; la 115 (unificación Tipo de OS/Trabajo) está escrita y validada
read-only pero pendiente de aplicar al momento de escribir esto — cuando se aplique,
`tipos_os` se dropea y `tipos_trabajo` se renombra a `tipos_os_trabajo`, quedando en 87
tablas netas—. Del 101 al 114: todas aditivas salvo la 113 (cambia el tipo de una columna,
sin tabla nueva) — folios propios (Cita/Viaje/Levantamiento en la 108; Cliente/Pack de
...
texto_encabezado` texto→jsonb (113), entre otras).
**Las 88 tienen RLS activo** (100%) — 63 con policy de tenant real (`empresa_id =
